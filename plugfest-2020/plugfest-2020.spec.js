    ...
    describe(vendor.name, () => {
      ...
      describe('Verifier', () => {
        describe('Verify Credential HTTP API', () => {
          ...
          // eslint-disable-next-line-max-len
          describe.skip('14. The Verifier\'s Verify Credential HTTP API SHOULD verify all other teams credentials', () => {
            const endpoint = vendor.verify_credential_endpoint;
            getVendors().filter(ven => ven.name !== vendor.name).map(ven => {
        ...
        describe('Verify Presentation HTTP API', () => {
          ...
          // eslint-disable-next-line-max-len
          describe('7. The Verifier\'s Verify Credential HTTP API SHOULD verify all other teams credentials', () => {
            const endpoint = vendor.verify_presentation_endpoint;
            getVendors().filter(ven => ven.name !== vendor.name).map(ven => {
              it(`should verify ${ven.name}\'s credentials`, async () =>{
                await Promise.all(ven.verifiable_presentations.map(async vp => {
                  const body = {
                    verifiablePresentation: vp,
                    options: {
                      challenge: vp.proof.challenge,
...
function getVendors() {
  const transmute = require('./vendors/transmute');
  const digital_bazaar = require('./vendors/digital_bazaar');
  const danubetech = require('./vendors/danubetech');
  const mavennet = require('./vendors/mavennet');
  const trustbloc = require('./vendors/trustbloc');
  const factom = require('./vendors/factom');
  const sicpa = require('./vendors/sicpa');
  ...
  return Object.values(vendor_map);
}
